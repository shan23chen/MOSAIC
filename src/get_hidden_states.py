...
def get_hidden_states(
    ...
    model, tokenizer_or_processor, sae = load_models_and_tokenizer(checkpoint, layer, device, model_type, sae_release)
    model.eval()

    # Load the dataset
    logging.info(f"Loading dataset: {dataset_name}, config: {dataset_config_name}, split: {dataset_split}")
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, collate_fn=custom_collate_fn)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(data_loader, total=total_batches, desc="Processing Batches")):
            if max_batches is not None and batch_idx >= max_batches:
                logging.info(f"Reached maximum number of batches ({max_batches}). Stopping.")
                break
            try:
                ...
                if model_type == "vlm":
                    ...
                    if images is None:
                        raise ValueError(f"Image field '{image_field}' not found in the dataset.")
                    texts = [''] * len(images)
                    inputs = prepare_inputs(images, texts, tokenizer_or_processor, device, model_type)
                else:
                    texts = batch.get(text_field)
                    if texts is None:
                        raise ValueError(f"Text field '{text_field}' not found in the dataset.")
                    inputs = prepare_inputs(None, texts, tokenizer_or_processor, device, model_type)
                ...
                retrieve_sae_features(model_name, layer, hidden_states, labels, ids, sae, save_dir=save_dir, activation_only=activation_only)
