...
def retrieve_sae_features(model_name, layer, hidden_states, labels, ids, sae, save_dir="./npz_files", activation_only=True):
    os.makedirs(save_dir, exist_ok=True)
    results = []

    with torch.no_grad():
        for idx_in_batch, (hidden_state, label, sample_id) in enumerate(tqdm(zip(hidden_states, labels, ids), total=len(ids), desc="Processing Samples in Batch")):
            try:
                if activation_only:
                    # Store the activations as an npz file
                    file_id = f"sample_{sample_id}"
                    npz_file_path = os.path.join(save_dir, f"{file_id}.npz")
                    np.savez_compressed(npz_file_path, hidden_state=hidden_state.cpu().numpy())
                    ...
                    file_id = f"sample_{sample_id}"
                    npz_file_path = os.path.join(save_dir, f"{file_id}.npz")
                    np.savez_compressed(npz_file_path, sae_acts=sae_acts.cpu().numpy(), hidden_state=hidden_state.cpu().numpy())
    ...
    if activation_only:
        ...
        results_csv_path = os.path.join(save_dir, f"{model_name}_{layer}_activations_metadata.csv")
    else:
        results_df = pd.DataFrame(results)
        results_csv_path = os.path.join(save_dir, f"{model_name}_{layer}_sae_activations_metadata.csv")
